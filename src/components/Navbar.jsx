        ...
        {navItems.map((item) => (
          ...
          >
            {item.icon === 'droplet' ? (
              ...
              <i className={`${item.iconClass} flex items-center justify-center w-5 h-5 text-lg flex-shrink-0`}></i>
            )}
            <span className="text-left line-clamp-2 flex items-center">{item.label}</span>
          </button>
        ))}
      </div>
      <button
        ...
        style={{ minHeight: 48 }}
      >
        <i className="fi fi-rr-sign-out-alt flex items-center justify-center w-5 h-5 text-lg"></i>
